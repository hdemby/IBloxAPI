...
@myprofilefunc
def getinfo(network, cidr):
	"""return the host from an IP address"""
	search = search_patt.format(network, cidr)
...
if __name__ == '__main__':
	...
	try:
		if sys.argv[1] !=[]:
			network, cidr  = sys.argv[1:]
		else:
			raise IndexError
	except IndexError:
		network, cidr = raw_input("what network and CIDR?(network): ").split("/")
		
	print("the results for: '{}/{}'\n".format(network, cidr))
	search = search_patt.format(network, cidr)
	ans = main(network, cidr)
